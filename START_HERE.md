...
当前阶段：针对“安全账户”转账请求的风险证据与双人确认协议原型，不是生产 SDK。麦克风风险建议和文本确认仍是两个未连通的演示。真实身份登记、远程可信通道和真实工具拦截尚未实现。

## 试用本地核验闭环

在项目 PowerShell 中执行 `./Start-Review-Demo.ps1`，保持窗口打开。它自动选择已有环境，打印两个完整链接，不安装依赖、不调用云端。

1. 打开 Participant 链接（8766）：允许处理虚构测试内容，分析台词，再提交给确认者。
...
演示入口带随机权限凭据，批准还需要 Participant 页面单独显示的一次性挑战码，三次错误会取消请求。两个进程使用不同权限，签名私钥只在 Reviewer 进程生成；这只是双人确认原型，没有登记真人身份或已知联系方式。整台电脑及两个后端仍需可信。按 Ctrl+C 停止；重启后旧入口和请求失效。此入口暂用文本回放，与 8765 麦克风页面分开。
