...
会话控制层已提供本地连接演示：在完整环境运行 `python -m scripts.workflow_demo`。
同一个会话中的安全账户请求进入 CHALLENGED，再由测试密钥确认具体金额和目标的摘要，最终执行模拟操作。
新转录会使待确认请求失效；COOLING_OFF/BLOCKED 不允许创建确认。
此演示仍使用进程内测试核验者，没有真人注册、独立确认页面或实时麦克风接入。
本次完整回归为 94 项通过（此前生成的本地报告保留其原始运行结果）。
