...
日期：2026-09-09，接入状态更新于 2026-09-10。用途：统一实现、评测与项目讲述。本文定义框架和验收性质，并非已经完成形式化证明或真实场景有效性验证。当前交付以 [交付报告](DELIVERY_STATUS.md) 为准。
...
图表示目标连接关系。当前已把语音/文本、风险状态、分角色挑战确认和模拟执行连接；证据图与签名风险回执可从同一会话导出。真人身份登记与已登记带外渠道仍未实现，流程无法证明由两个不同真人操作。
...
| 图 | NetworkX 当前会话/回放图、去重权重解释与来源引用测试 | 历史锁定原因、实体解析 |
| 核验 | 登记测试公钥、签名确认、范围/过期/篡改测试 | 真人注册、可信界面、撤销和密钥管理 |
| 执行 | 与语音策略连接的模拟 gate、另有 SQLite 并发与跨进程测试 | 真人确认来源、真实工具事务 |
| 概率/信息论 | 混淆矩阵、开发集比率与 Wilson 区间；推断定义与候选库 | 独立样本、参数、校准与有效性实验 |
| GNN/ZK/激励 | 研究候选 | 当前无集成；按产品需求再决定 |

测试数量见带生成日期的 [本地报告](../scambench/LOCAL_RESULTS.md)。历史 25 个同作者合成开发案例得到 TP=15、FP=0、FN=1、TN=9；提醒召回 93.75% 的 Wilson 95% 区间为 71.7%–98.9%，特异度 100% 的区间为 70.1%–100%。宽区间直接显示样本很小；这些数字是开发回归结果，不是现实准确率或数学证明。签名凭据只能证明快照未被篡改，不能证明风险判断正确。
